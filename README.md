...
- **Launcher key.** Tap to send Super/Meta. On Plasma, this opens Application
  Launcher by default. Hold and release to keep Super active for a shortcut.
  With `auto` selected, the icon matches Bazzite or Arch and uses Tux on other
  systems. Select an icon from tray
  **Settings ▸ Launcher key icon** or `handheld-kbd-ctl super-icon <auto|bazzite|windows|arch|tux>`.
  The Bazzite icon comes from its [official press kit](https://github.com/ublue-os/bazzite/tree/main/press_kit).
...
On ASUS ROG Ally (RC71L) and Ally X (RC72LA), M1 shows or hides the keyboard when
HHD is active. M2 keeps its HHD action. InputPlumber and Steam Input are not
required.

Do not assign another desktop action to physical F17 on these devices.
M1 sends F17, which Plasma can identify as **Launch (8)**. The keyboard removes
duplicate shortcuts that use this key alone to start `handheld-kbd-toggle`.

If M1 does not respond, use `handheld-kbd-ctl status`.
This shows the controller service and selected input device.

### Touchscreen text fields

On Plasma Wayland, the installer selects **Better Handheld Keyboard** in
**System Settings → Virtual Keyboard**. If you later select another virtual
keyboard, upgrades keep that selection.

To open this keyboard when you tap a text field, keep **Better Handheld
Keyboard** selected. The application must support Wayland text input.
For other applications, use `handheld-kbd-toggle`. On a supported Ally,
M1 also works in these applications.

After an upgrade, log out and back in to reload the Plasma virtual keyboard.
The installer keeps your keyboard configuration. Uninstall and
`handheld-kbd-recover --stock-only` restore the previous virtual keyboard.

### Bottom margin

To leave a gap below the docked keyboard, select tray
**Settings ▸ Bottom margin** or use:

```bash
handheld-kbd-ctl set dock_bottom_margin 40 int
```

The default margin is 0 logical pixels. Logical pixels account for display
scaling. The margin applies to all docked sizes and leaves custom positions
unchanged. The command restarts the keyboard.
...
With InputPlumber, the button sends a **DBus event** instead of a keystroke, so
